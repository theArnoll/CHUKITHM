...
void loop() {
    ...
    if(digitalRead(7) == LOW)
    {
        ...
        for (uint8_t re = 0; re < 2; re++)
        {
            ...
            {
                uint8_t pressed = touchStatus[re] >> pointer & 0b00000001;
    ...
    {   for (uint8_t bfrsel = 0; bfrsel <= 1; bfrsel++)
        {   for (int pointer = 0; pointer < 8; pointer++)
            {   uint8_t pressed = touchStatus[bfrsel] >> pointer & 0b00000001,
                    lastPressed = lastStatus[bfrsel]  >> pointer & 0b00000001;
                if(pressed && (pressed != lastPressed))
                {
                    KB.add(keymap[bfrsel][pointer]);
                    if(!digitalRead(7)) Serial.println(keymap[bfrsel][pointer]);
                }
                else {
                    if(pressed != lastPressed)
                    {
                        KB.release(keymap[bfrsel][pointer]);
                        if(!digitalRead(7)) { Serial.print("-"); Serial.println(keymap[bfrsel][pointer]); }
                    }
                }
            }  // end for in byte
        }  // end for in buffer
...
void requestTouchStatus(uint8_t statusStorage[2])
{
    ...
    if (error != 0)
    {
        ...
        printError(error);
        for (uint8_t re = 0; re < 2; re++)
            touchStatusBuffer[re] = (statusStorage[re] == 0x55 ? 0xAA : 0x55);
    }
    statusStorage[0] = touchStatusBuffer[0]; statusStorage[1] = touchStatusBuffer[1];
}
