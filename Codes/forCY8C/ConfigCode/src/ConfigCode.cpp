...
#define I2C_ADDRESS 0x52     //I2C Address of the Cypress IC (0x37 is default)
#define REQUEST_TIMEOUT 40   //After how many request is a Timeout triggered
#define NEW_I2C_ADDRESS 0x52 //I2C Address after programm. IC needs to be reseted for changes to apply
...
uint8_t configureSensors() {  // SENSOR_EN, FFS_EN, SENSIVITY, THRESHOLD
    ...
    uint8_t THRESHOLD_BUFFER[16] = { 140 };  //From index 0-15 the threshold value for sensor 0-15
    ...
    Serial.println("+ Set SPO settings");
    uint8_t val_SPO_CFG = 0x10; // Make CS15 become CS and no longer HI'
    error = touchIC.set_SPO_CFG(&val_SPO_CFG); 
    if (error != 0) {
        return error;
    }

    Serial.println("+ 強制關閉系統自檢 (DEVICE_CFG1)");
    // 根據 TRM 第 90 頁，DEVICE_CFG1 的 Bit 0 是 SYSD_EN (預設為 1)
    // 設為 0x00 代表強行關閉自我診斷，不准晶片自動關閉寄生電容過大的按鍵！
    uint8_t val_CFG1 = 0x00; 
    error = touchIC.set_DEVICE_CFG1(&val_CFG1);
...
void setup() {
    ...
    Serial.println("Waking up Cypress IC...");
    Wire.beginTransmission(I2C_ADDRESS); 
    Wire.endTransmission();  // 發送空信號（敲門），此時會收到 NACK 但沒關係
    delay(50);               // 強制等待 50 毫秒，等晶片完全清醒
    configueTouchIC();
    Wire.beginTransmission(I2C_ADDRESS);
    Wire.write(0x86);
    Wire.write(0x02);
